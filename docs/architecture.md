...
environment data, BVH nodes, and leaf primitive references use flat storage
buffers so the shader can index them
...
This design makes buffer sizes and transfers predictable and keeps the CPU and
GPU BVH representations structurally aligned. The cost is more manual
